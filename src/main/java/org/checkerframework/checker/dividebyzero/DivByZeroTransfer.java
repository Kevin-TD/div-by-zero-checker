...
// https://github.com/kelloggm/div-by-zero-checker/blob/master/INSTRUCTIONS.md
// ./gradlew build
...
public class DivByZeroTransfer extends CFTransfer {
    ...
    private AnnotationMirror refineLhsOfComparison(
        ...
        // System.out.println("refinement " + operator + " " + lhs.toString() + " " + rhs.toString());
        
        // type refinement only useful if it's top (?)
        if (!equal(lhs, top())) return lhs; 
        ...
        else if (operator == Comparison.LT || operator == Comparison.GT) {
            if (equal(rhs, zero())) return nonZero(); 
        }
    ...
    private AnnotationMirror arithmeticTransfer(
        ...
        // System.out.println("divbyzerotransfer " + operator + " " + lhs.toString() + " " + rhs.toString());
        ...
        if (operator == BinaryOperator.PLUS) {
            ...
            if (!equal(lhs, rhs)) return nonZero();
            if (equal(lhs, zero())) return zero();
            return top(); 
        }
        else if (operator == BinaryOperator.TIMES) {
            if (equal(lhs, rhs) && equal(lhs, nonZero())) return nonZero(); 
            return zero(); 
        }
